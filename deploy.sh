...
if [[ "$confirm" =~ ^[Yy]$ ]]; then
  if gcloud beta builds triggers describe vuejs-template \
      ...
      LATEST_SECRET_CONTENT=$(gcloud secrets versions access latest --secret="vuejs-template")
      ...
      if [ "$LATEST_SECRET_CONTENT" != "$CURRENT_ENV_CONTENT" ]; then
        ...
        for VERSION_ID in $(gcloud secrets versions list vuejs-template --filter="state != disabled" --format="value(name)" --sort-by="~createTime"); do
          gcloud secrets versions disable $VERSION_ID --secret="vuejs-template"
        done
        gcloud secrets versions add vuejs-template --data-file=$env_file
      else
        printf "$env_file content matches last secret version. Skipping env update...\n"
      fi
      printf "INFO: Trigger found, deploying...\n"
      gcloud beta builds triggers run vuejs-template \
        --project="$GCLOUD_PROJECT_ID" \
        --region="europe-west9" \
        --branch="main" > /dev/null && echo "Cloud Build trigger has been run: see at https://console.cloud.google.com/cloud-build/builds;region=europe-west9?authuser=0&project="$GCLOUD_PROJECT_ID"&supportedpurview=project"
  else
    ...
    gcloud secrets create vuejs-template \
    ...
    gsutil mb -c standard -l europe-west9 gs://$GCLOUD_PROJECT_ID-tfstate

    # Create required artifact registry for Cloud Run image
    printf "\nINFO: Creating Artifact registry repository...\n"
    gcloud artifacts repositories create "vuejs-template-repository" \
      --location=europe \
      --repository-format=docker \
      --description="Images for building vuejs-template Cloud Run service"
    ...
    gcloud beta builds triggers create github \
        ...
        --project="$GCLOUD_PROJECT_ID" \
        --repo-owner="$(echo "ahourlier/vuejs_template" | cut -d'/' -f1)" \
        --repo-name="$(echo "ahourlier/vuejs_template" | cut -d'/' -f2)" \
